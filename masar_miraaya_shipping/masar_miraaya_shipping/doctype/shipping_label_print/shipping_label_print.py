...
class ShippingLabelPrint(Document):
    ...
    def generate_qrcodes(self):
        try:
            ...
            magento_ids = [o.magento_id for o in self.orders if o.magento_id]
            label_cache = {}
            if magento_ids:
                label_cache = {
                    r.magento_id: r.base64_image
                    for r in frappe.get_all(
                        "Shipping Label Image",
                        filters={
                            "magento_id": ["in", magento_ids],
                            "status":     "Completed",
                        },
                        fields=["magento_id", "base64_image"],
                    )
                }
            ...
            for order in self.orders:
                ...
                # ── Outsourced ────────────────────────────────────────────
                    ...
                    if expected_dc_name and expected_dc_name != order.delivery_company_name:
                        ...
                        if not cont:
                            continue
                        # Invalidate cache for this order — label may have changed after reassign
                        label_cache.pop(order.magento_id, None)

                    # Try cache first
                    cached_image = label_cache.get(order.magento_id)

                    if cached_image:
                        order.outsourced_label = f"data:image/png;base64,{cached_image}"
                    else:
                        # Fallback: fetch live if not pre-cached (e.g. reassigned or new order)
                        ...
                        if label_pdf:
                            ...
                            if base64_image:
                                ...
                                # Back-fill the cache doctype so next time it's instant
                                create_label_image(order.magento_id, order.sales_order, base64_image)
...
def create_label_image(magento_id, sales_order, base64_image):
    """Write-through cache: keep Shipping Label Image in sync."""
    existing = frappe.db.get_value(
        "Shipping Label Image", {"magento_id": magento_id}, "name"
    )
    if existing:
        frappe.db.set_value(
            "Shipping Label Image", existing,
            {"base64_image": base64_image, "status": "Completed",
