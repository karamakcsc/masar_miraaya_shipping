frappe.pages['driver-assignment'].on_page_load = function(wrapper) {
	...
	function render_scanned_list() {
		...
		if (total_count === 0) {
			$orders_container.append(
				'<p class="text-muted text-center">Scan sales orders to dispatch</p>'
			);
			return;
		}
		...
		frappe.call({
			...
			callback: function(r) {
				if (r.message.status === 'error') {
					frappe.show_alert({
						message: scan_errors[r.message.reason] || __('Sales Order cannot be dispatched'),
						indicator: 'red'
					});
					return;
				}
				...
				frappe.show_alert({
					message: __('Sales Order {0} dispatched successfully', [r.message.data.sales_order]),
					indicator: 'green'
				});
	...
	function fetch_existing_orders() {
		...
		frappe.call({
			method: 'masar_miraaya_shipping.masar_miraaya_shipping.page.driver_assignment.driver_assignment.get_dispatched_orders',
			args: {
				driver: driver.get_value(),
