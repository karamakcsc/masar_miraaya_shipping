...
@frappe.whitelist()
def scan_and_set_delivered(sales_order, driver=None):
    so_name = sales_order
    ...
    if so.custom_magento_status == "Delivered":
        return {
            "status": "error",
...
@frappe.whitelist()
def get_delivered_orders(driver=None, from_date=None, to_date=None):
    if not driver:
        return []
    ...
    conditions = ["custom_magento_status = 'Delivered'"]
